...
def main():
    ...
    def cadastrar_entrada():
        ...
        try:
            ...
            return tkinter.messagebox.showerror(title="Aviso", message=f"A data inserida não está no formato correto!O formato aceita apenas números e barras,sendo dia/mês/ano.Exemplo:12/05/1999"),janela.focus_force()
        ...
        try:
            ...
            custo = float(quantidade)*float(produto[3])
            nova_quantidade = int(quantidade) + int(produto[2])
            print(produto)
        ...
        try:
            cursor.execute('''INSERT INTO estoque (operacao , id_produto , quantidade , data , custo )
                        VALUES (? , ?, ?, ?, ?)''', ("entrada" ,id_produto , quantidade, data , custo))
            conn.commit()
            tkinter.messagebox.showinfo(title="Aviso", message=f"Entrada cadastrada com sucesso!")
            conn.close()
        except:
            tkinter.messagebox.showerror(title="Aviso", message=f"Erro ao cadastrar a entrada!")
            janela.focus_force()
            conn.close()
...
#   botão para cadastro da entrada
    button_cadastrar = customtkinter.CTkButton(master=janela,text="Cadastrar",bg_color="black",command = cadastrar_entrada)
